...
pub fn handshake_listen(
    ...
    let framed = read(stream).and_then(move |s| {
        let mut buf = vec![0u8; 65535];
        // <- e
        noise.read_message(&s.1, &mut buf);

        // -> e, ee, s, es
        let len = noise.write_message(&[0u8; 0], &mut buf).unwrap();

        write(s.0, buf, len).and_then(|s| read(s.0)).and_then(
            move |s| {
                let readed_buf = s.1;
                let mut buf = vec![0u8; 65535];
                // <- s, se
...
pub fn noise_reader(stream: TcpStream) -> Box<Future<Item = (), Error = ()>> {
    let reader = handshake_listen(stream)
        .and_then(|framed| {
...
pub fn noise_writer(stream: TcpStream) -> Box<Future<Item = (), Error = io::Error>> {
    let writer = handshake_sender(stream)
        .and_then(|framed| {
